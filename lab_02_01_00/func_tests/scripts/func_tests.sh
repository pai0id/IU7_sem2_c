...
files=(../data/pos_[0-9][0-9]_in.txt)
if [ "$1" == "-v" ]; then
    echo "Pos tests"
fi
for input in "${files[@]}"
do
    if [ "$input" == "../data/pos_[0-9][0-9]_in.txt" ]; then
        if [ "$1" == "-v" ]; then
            echo "-"
        fi
        break
    ...
    out_file=../data/pos_"$curr_num"_out.txt
...
files=(../data/neg_[0-9][0-9]_in.txt)
if [ "$1" == "-v" ]; then
    echo
    echo "Neg tests"
fi
for input in "${files[@]}"
do
    if [ "$input" == "../data/neg_[0-9][0-9]_in.txt" ]; then
        if [ "$1" == "-v" ]; then
            echo "-"
        fi
        break
    ...
    curr_num=$(echo "$input" | grep -o '[0-9][0-9]')
    out_file=../data/neg_"$curr_num"_out.txt
